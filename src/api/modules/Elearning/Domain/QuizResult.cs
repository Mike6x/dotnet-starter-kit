...
public class QuizResult : AuditableEntity, IAggregateRoot
{
        ...
        Guid? quizId)
    {
        ...
        if (timeTakingQuiz is not null && Fut?.Equals(timeTakingQuiz, StringComparison.Ordinal) is not true) Fut = timeTakingQuiz;

        if (quizTitle is not null && Qt?.Equals(quizTitle, StringComparison.Ordinal) is not true) Qt = quizTitle;
        ...
        if (quizVersion is not null && V?.Equals(quizVersion, StringComparison.Ordinal) is not true) V = quizVersion;
        if (quizType is not null && T?.Equals(quizType, StringComparison.Ordinal) is not true) T = quizType;
        ...
        QueueDomainEvent(new QuizResultUpdated(this));
