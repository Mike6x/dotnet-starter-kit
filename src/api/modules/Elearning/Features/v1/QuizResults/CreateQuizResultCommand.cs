...
public record CreateQuizResultCommand(
    
    [property: DefaultValue(null)] DateTime? StartTime,
    [property: DefaultValue(null)] DateTime? EndTime,
    ...
    Guid QuizId
    // [property: DefaultValue("string.Empty")] string QuizName, 
    // [property: DefaultValue("string.Empty")] string QuizCode
   ) : IRequest<CreateQuizResultResponse>;

public record CreateQuizResultResponse(Guid? Id);

public sealed class CreateQuizResultHandler(
    ILogger<CreateQuizResultHandler> logger,
    [FromKeyedServices("elearning:quizresults")] IRepository<QuizResult> repository)
    : IRequestHandler<CreateQuizResultCommand, CreateQuizResultResponse>
{
    public async Task<CreateQuizResultResponse> Handle(CreateQuizResultCommand request, CancellationToken cancellationToken)
    {
...
{
    public CreateQuizResultValidator(ElearningDbContext context)
    {
        RuleFor(e => e.QuizId)
            .NotEmpty()
