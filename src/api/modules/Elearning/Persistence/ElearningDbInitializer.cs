...
internal sealed class ElearningDbInitializer(
    ILogger<ElearningDbInitializer> logger,
    ElearningDbContext context) : IDbInitializer
{
    public async Task MigrateAsync(CancellationToken cancellationToken)
    {
        if ((await context.Database.GetPendingMigrationsAsync(cancellationToken).ConfigureAwait(false)).Any())
        {
            ...
            logger.LogInformation("[{Tenant}] applied database migrations for Elearning module", context.TenantInfo!.Identifier);
        }
    }
